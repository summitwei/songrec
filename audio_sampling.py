...
def analog_to_digital(ax, sampling_rate, quantizing_bits, digital_graph=False):
    import numpy as np

    x = 2.0
    analog_time = np.linspace(0, x, int(10e5))
    ...
    if not digital_graph:
        ...
        ax.stem(
            sampling_time, quantizing_signal, markerfmt="r_", linefmt=" ", basefmt=" "
        )
